...
public class Shader {
    ...
    private static int loadShader(String name) {
        ...
        String code = readInputStream(ResourceUtil.loadStream("/shaders/" + name));
    ...
    private static int compileShader(int type, String code) {
        ...
        if (glGetShaderi(shader, GL_COMPILE_STATUS) == GL_FALSE) {
            ...
            throw new RuntimeException("Unable to compile " + typeStr + " shader." + glGetShaderInfoLog(shader, 1000));
        }
        return shader;
    }
    ...
    private static int linkProgram(int... shaders) {
        ...
        if (glGetProgrami(program, GL_LINK_STATUS) == GL_FALSE) {
            throw new RuntimeException("Unable to link shaders." + glGetProgramInfoLog(program, 1000));
        }
    ...
    private int findUniform(String name) {
        ...
        if (uniform == -1) {
            Logger.getGlobal().log(Level.INFO, "Uniform does not exists: {0}", name);
        }
        return uniform;
    }
